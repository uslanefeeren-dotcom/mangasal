...
def get_page(url, session):
    # Önce curl_cffi ile dene (Cloudflare bypass)
    if CURL_OK:
        for imp in ["chrome110", "chrome107", "chrome104"]:
            try:
                r = cffi_requests.get(
                    url, impersonate=imp, timeout=20,
...
def scheduler():
    # DB doluysa hemen scrape etme, 2 saat bekle
    try:
        _c = sqlite3.connect(DB)
        _n = _c.execute("SELECT COUNT(*) FROM series").fetchone()[0]
        _c.close()
    except:
        _n = 0
    if _n > 0:
        log.info("DB'de %d seri var, scrape %d saat sonra baslar", _n, INTERVAL)
...
def proxy_image():
    ...
    try:
        ...
        # curl_cffi ile dene
        if CURL_OK:
            for imp in ["chrome110", "chrome107"]:
                try:
                    r = cffi_requests.get(
                        img_url, impersonate=imp, timeout=15,
                    ...
                    continue
        # Fallback: normal requests stream
