    ...
    while page <= 100:
        ...
        if parser_type == "ragnar":
            ...
            next_btn = soup.select_one(
                "a.next, .nextpostslink, a[rel='next'], "
                "a.next.page-numbers, .nav-previous a"
            )
            if not next_btn and parser_type == "madara":
                page += 1
                time.sleep(DELAY)
                continue
            elif not next_btn:
...
def scrape_detail(url, session):
    ...
    if not chapters:
        ...
        for li in chapter_items:
            ...
            num_text = html_module.unescape(
                num_el.get_text(strip=True) if num_el else li.get("data-num","")
            )
            ...
            try:
                ...
                if ajax_r.ok and len(ajax_r.text) > 10:
                    ...
                    for li in ajax_soup.select("li"):
                        ...
                        num_text = html_module.unescape(
                            num_el.get_text(strip=True) if num_el else a.get_text(strip=True)
                        )
                        date = date_el.get_text(strip=True) if date_el else ""
                        chapters.append({"num": num_text, "title": num_text,
                                         "url": chap_url, "date": date})
